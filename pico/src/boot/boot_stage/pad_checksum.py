...
# struct : struct in c : pack("endian type", val)
#    < : little endian <: big endian l:long(4byte) L:unsigned long(4-byte) b,h(half word,2byte),i,l,q(quad,8byte)
odata = idata_padded + struct.pack("<L", checksum)

try:
    # with ~ as ~ : auto free resource
